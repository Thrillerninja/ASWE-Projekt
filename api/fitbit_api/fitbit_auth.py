...
class FitbitAuth:
    ...
    def __init__(self):
        ...
        self.TOKEN_FILE = os.path.join(os.path.dirname(__file__), 'fitbit_tokens.json')  # Dynamisch den Pfad setzen
        self.load_tokens()
    ...
    def refresh_access_token(self):
        if self.refresh_token:
            ...
            print("Kein Refresh Token vorhanden. Bitte authentifizieren Sie sich erneut.")

    def authorize(self):
        state = str(uuid.uuid4())
        fitbit = OAuth2Session(self.CLIENT_ID, scope=self.SCOPE, redirect_uri=self.REDIRECT_URI, state=state)
        authorization_url, _ = fitbit.authorization_url(self.AUTHORIZATION_BASE_URL, prompt='consent')
